...
---
sequenceDiagram
    %% LEVEL 1 CONDENSED: Cross-System View với Function Info
    autonumber
    ...
    %% Systems
    participant LUK as Hệ thống Luklak LAIKA
    participant EXT as Hệ thống Bên ngoài<br/>(Website/Email/Google/Sepay/VOIP)
    
    %% Marketing Flow
    KH->>+EXT: Điền form đăng ký
    EXT->>+LUK: [Webhook] → Tạo [Marketing/🧊 Lead]
    LUK-->>+EXT: [Marketing/🧊 Lead/CHUYỂN CRM] → Gửi email chào mừng
    EXT-->>KH: Email chào mừng
    deactivate EXT
    deactivate LUK
    deactivate EXT
    ...
    %% Sales Flow
    activate NV
    NV->>+LUK: Tạo lịch hẹn với khách
    LUK-->>+EXT: [CRM/🧊 Lịch hẹn/ĐẶT LỊCH] → Tạo Google Calendar
    EXT-->>KH: Lời mời họp
    deactivate EXT
    deactivate LUK
    deactivate NV
    ...
    KH->>+EXT: Gọi hotline
    EXT-->>+LUK: [Webhook] → Log vào [CRM/🧊 Khách hàng]
    deactivate LUK
    deactivate EXT
    
    activate NV
    NV->>+LUK: Click gọi khách
    LUK-->>+EXT: [CRM/🧊 Khách hàng/Menu: Gọi] → VOIP call
    EXT-->>KH: Cuộc gọi đến
    deactivate EXT
    deactivate LUK
    deactivate NV
    ...
    activate NV
    NV->>+LUK: Soạn hợp đồng
    LUK-->>+EXT: [Contract/🧊 Hợp đồng/GỬI KHÁCH] → Tạo Google Docs
    EXT-->>LUK: Link hợp đồng
    deactivate EXT
    LUK-->>KH: Link hợp đồng
    
    LUK-->>+EXT: [Contract/🧊︎ Phiếu thu/CẦN THU] → Tạo QR Sepay
    EXT-->>LUK: Mã QR
    deactivate EXT
    LUK-->>KH: Mã QR thanh toán
    deactivate LUK
    deactivate NV
    
    KH->>+EXT: Thanh toán QR
    EXT-->>+LUK: [Webhook] → Update [Contract/🧊︎ Phiếu thu/ĐÃ THU]
    deactivate LUK
    deactivate EXT
    
    %% Project Flow
    activate NV
    NV->>+LUK: Khởi động dự án
    LUK-->>+EXT: [Product Delivery/🧊 Dự án/KICK OFF] → Tạo Google Drive folder
    deactivate EXT
    deactivate LUK
    deactivate NV
    ...
    activate NV
    NV->>+LUK: Gửi thiết kế
    LUK-->>+EXT: [Product Delivery/🧊 Thiết kế/SEND CUSTOMER] → Email concept
    EXT-->>KH: Email thiết kế
    deactivate EXT
    deactivate LUK
    deactivate NV
    ...
    %% Customer Care Flow
    activate LUK
    LUK-->>+EXT: [Customer Success/🧊 CSKH/sau 60 ngày] → Email khảo sát
    EXT-->>KH: Email survey
    deactivate EXT
    deactivate LUK
    
    KH->>+EXT: Điền feedback
    EXT-->>+LUK: [Webhook] → Update [Customer Success/🧊 CSKH]
    deactivate LUK
    deactivate EXT

%% Format: [Function/Object/Status or Trigger]
%% Marketing, CRM & Sales, Contract & Invoice, Product Delivery, Customer Success
```