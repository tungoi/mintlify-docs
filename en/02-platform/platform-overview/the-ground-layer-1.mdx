...
Once inside a specific building (like a `⏹️ Space`), the **Permission Scheme** governs what a user can actually _do_ to the contents (the `🧊 Objects`) within. This is the most granular layer of control.
...
Then, in a live `⏹️ Space`, a manager assigns specific `Users` or `Groups` to the `Roles` that were defined in the `Function`'s blueprint. This provides ultimate flexibility: the rules are standardized, but _who plays which role_ can be different for each individual `Space`.
