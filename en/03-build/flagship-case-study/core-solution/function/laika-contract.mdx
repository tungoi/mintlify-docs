...
<Tabs>
  <Tab title="Workflow">
    **Contract Workflow**
    ...
    `DRAFT` → `PENDING APPROVAL` → `SENT TO CLIENT` → `SIGNED` → `IN PROGRESS` → `COMPLETED` → `LIQUIDATED`
...
### 2. 🧊︎ Payment Installment (Sub-Object)
...
| Rule                               | Trigger                                               | Action                                                                                                                                                    |
| :--------------------------------- | :---------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1. **Create Installment Schedule** | A `🧊 Contract` is moved to `SIGNED`.                 | Reads the payment terms and automatically creates the required number of `🧊︎ Payment Installment` sub-objects with correct due dates and amounts.        |
| 2. **Send Payment Reminders**      | An automation runs daily.                             | Checks for any `🧊︎ Payment Installment` where the `Due Date` is today and the `Status` is `NOT DUE`. It changes the status to `DUE` and sends reminders. |
| 3. **Update Contract Status**      | The `Remaining Balance` on a `🧊 Contract` becomes 0. | Automatically changes the `🧊 Contract` status to `COMPLETED`.                                                                                            |
